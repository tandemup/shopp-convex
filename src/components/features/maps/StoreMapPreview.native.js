...
export default function StoreMapPreview({
  ...
  function parkingIcon(size){const style=${JSON.stringify(parkingMarkerStyle)},color=${JSON.stringify(parkingMarkerColor)};if(style==='circle')return roundIcon(color,'P',size);if(style==='circle-stick'){const reducedSize=size/3,h=reducedSize*1.5;return L.divIcon({className:'',html:'<svg width="'+reducedSize+'" height="'+h+'" viewBox="0 0 30 46" xmlns="http://www.w3.org/2000/svg" style="filter:drop-shadow(0 3px 3px rgba(15,23,42,.25))"><line x1="15" y1="18" x2="15" y2="44" stroke="#9ca3af" stroke-width="3" stroke-linecap="round"/><circle cx="15" cy="14" r="13" fill="'+color+'" stroke="#fff" stroke-width="2"/><circle cx="19" cy="9" r="3" fill="rgba(255,255,255,.55)"/></svg>',iconSize:[reducedSize,h],iconAnchor:[reducedSize/2,h],popupAnchor:[0,-h+4]});}const h=Math.round(size*1.38);return L.divIcon({className:'',html:'<svg width="'+size+'" height="'+h+'" viewBox="0 0 42 56" xmlns="http://www.w3.org/2000/svg" style="filter:drop-shadow(0 4px 4px rgba(15,23,42,.3))"><path d="M21 1C10.2 1 1.5 9.7 1.5 20.5C1.5 34.3 15.3 48.6 21 55C26.7 48.6 40.5 34.3 40.5 20.5C40.5 9.7 31.8 1 21 1Z" fill="'+color+'" stroke="#fff" stroke-width="2.5"/><circle cx="21" cy="20.5" r="7" fill="#fff"/></svg>',iconSize:[size,h],iconAnchor:[size/2,h],popupAnchor:[0,-h+6]});}
